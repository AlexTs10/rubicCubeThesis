...
class AlgorithmComparison:
    ...
    def __init__(
        ...
        self.scramble_generation = "random_no_consecutive_same_face_moves"
        self.scramble_corpus_status = (
            "Generated by AlgorithmComparison.run_batch_test with allow_redundant=False."
        )
        self.current_generator_for_new_runs = "random_no_consecutive_same_face_moves"
    ...
    @staticmethod
    def _external_exact_backend_provenance() -> Optional[Dict[str, Any]]:
        """Return best-effort immutable provenance for the optional exact backend."""
        try:
            dist = importlib.metadata.distribution("RubikOptimal")
        except importlib.metadata.PackageNotFoundError:
            return None
        ...
        for file in dist.files or []:
            ...
            if file_name.endswith(".dist-info/METADATA"):
                key = "dist_metadata_sha256"
            elif file_name.endswith(".dist-info/RECORD"):
                key = "dist_record_sha256"
            elif file_name.endswith(".dist-info/LICENSE"):
                key = "license_file_sha256"
            elif file_name == "optimal/solver.py":
                key = "solver_py_sha256"
            if key is None:
                continue
        ...
        return {
            ...
            "import_package": "optimal",
            **hashes,
            "upstream_repository": "https://github.com/hkociemba/RubiksCube-OptimalSolver",
            "upstream_commit": None,
            "note": (
                "Installed PyPI distribution metadata does not expose an upstream "
                "commit or formal license field."
            ),
        }
    ...
    def export_results(self, filename: str) -> None:
        ...
        data = {
            'metadata': {
                ...
                'scramble_generation': self.scramble_generation,
                'scramble_corpus_status': self.scramble_corpus_status,
                'current_generator_for_new_runs': self.current_generator_for_new_runs,
                ...
                'external_exact_backend_provenance': self._external_exact_backend_provenance(),
            },
